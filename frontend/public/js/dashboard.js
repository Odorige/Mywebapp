...
async function loadDashboard() {
  try {
    ...
    renderGrid(data.videos, 'Latest videos');
  } catch (err) {
    grid.innerHTML = '';
    emptyState.style.display = 'block';
    emptyState.textContent = `We could not load the video feed: ${err.message}`;
  }
}
...
async function runSearch() {
  ...
  try {
    ...
    renderGrid(data.videos, q || genre ? 'Results' : 'Latest videos');
  } catch (err) {
    grid.innerHTML = '';
    emptyState.style.display = 'block';
    emptyState.textContent = `We could not complete that search: ${err.message}`;
  }
}
